...
export function useActivityTracker() {
    ...
    async function restore() {
      ...
      try {
        ...
        if (result.activity) {
          ...
          saveSession({ kind: 'personal', localId: result.activity.id, updatedAt: Date.now() })
        }
      } catch (caught) {
        setError(caught instanceof Error ? caught.message : String(caught))
  ...
  const start = useCallback(async (type: ActivityType, profile: TrackingProfile) => {
    ...
    if (nativeGpsAvailable()) {
      ...
      commitActivity({ id, eventId: null, type, status: 'ACTIVE', profile, startedAt, pausedAt: null, endedAt: null, pauseDurationMs: 0, rawDistanceM: 0, filteredDistanceM: 0, movingTimeMs: 0, stoppedTimeMs: 0, pointCount: 0 })
    }
    pointsRef.current = []
    setPoints([])
    setRecoveredCount(0)
  ...
  return { activity, points, diagnostics, recoveredCount, ready, error, setError, start, pause, resume, end, reset, reconcile }
}
